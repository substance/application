    ...
    while (worklist.length) {
      ...
      estraverse.replace(ast, {
        enter: function (node, parents) {
          ...
          try {
            var p = resolveOptions.path;
            if (p.indexOf('/') >= 0) {
              var p0 = path.join(root, path.dirname(node.loc.source), p);
              p0 = path.relative(root, p0);
              if (p0.indexOf('.js') < 0) {
                p0 += '.js';
              }
              // if (p.indexOf('native_shell') >= 0) {
              //   console.log("################", node.loc.source, p0);
              // }
              p = p0;
            }
            if(options.ignores && options.ignores[p]) {
